...
class MeetingService {
  ...
  public createMeeting = async (user: (User & mongoose.Document), createMeetingData: CreateMeetingDto) => {
    ...
    await email.sendTemplateEmail(EmailTemplate.Invitation, [{ email: recipient.email , name: recipient.name }], { FIRSTNAME: recipient.name.split(' ')[0], APPURL: process.env.APP_URL, _SI: authentication._si }, { name: process.env.APP_NAME, email: process.env.NOREPLY_EMAIL });
      ...
      if(meeting.initiator === user._id || meeting.recipient === user._id) { // FLOW: Is editing user a part of the meeting
        ...
        if(editMeetingData.status) {
          ...
          if(editMeetingData.status === MeetingStatus.Accepted) { // FLOW: If status is edited to Accepted, edit gcal invite to reflect this
            ...
            await email.sendTemplateEmail(EmailTemplate.Accepted, [{ email: user.email , name: user.name }], { FIRSTNAME: user.name.split(' ')[0], APPURL: process.env.APP_URL, _SI: authentication._si }, { name: process.env.APP_NAME, email: process.env.NOREPLY_EMAIL });
            ...
            await email.queueEmail({ templateId: EmailTemplate.PostEvent, to: [{ email: user.email , name: user.name }], params: { FIRSTNAME: user.name.split(' ')[0], APPURL: process.env.APP_URL, _SI: authentication._si }, sender: { name: process.env.APP_NAME, email: process.env.NOREPLY_EMAIL }, user: user }, { delay: delay });
